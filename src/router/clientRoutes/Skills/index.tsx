...
const SkillInformation = () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (data.data.applicantSkills.length > 0) {
            setFormValues({
              ...formValues,
              ...data.data.applicantSkills[0],
              skillId: data.data.applicantSkills[0].id,
            });
          }
        }
      } catch (error: any) {
  ...
  if (!hasProfile) {
    return (
      <Alert
        message={"You have to add profile before adding skills"}
        type="error"
      />
    );
  }
          ...
          <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
            <thead>
              <tr className="bg-gray-200 text-lg">
                <th className="border border-gray-300 text-center py-4">
                  Skill Name
                </th>
                <th className="border border-gray-300 text-center py-4">
                  Type
