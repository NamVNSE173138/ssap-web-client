...
const AccountDialog = ({ open, onClose, applications, scholarship }: any) => {
  ...
  const winnersApplications = applications.filter(
    (application: any) =>
  ...
  return (
    <Dialog onClose={onClose} open={open} fullWidth>
      <DialogTitle className="flex justify-between items-center bg-gradient-to-r from-blue-500 via-teal-400 to-blue-400 text-white p-5 shadow-md">
        ...
        {applications.length !== 0 && (
          <button
            ...
            className="flex items-center gap-3 bg-white text-blue-600 hover:bg-[#1eb2a6] hover:text-white transition-all duration-300 px-5 py-2 rounded-lg shadow-md active:scale-95"
          >
      ...
      <List sx={{ pt: 0 }} className="bg-gray-100 rounded-b-lg">
        ...
        {(selectedTab == 0 ? submittingApplications : winnersApplications)
          .length === 0 ? (
          <p className="p-10 text-center text-gray-500 font-semibold text-xl">
            No applications in this tab
          </p>
        ) : (
          (selectedTab == 0 ? submittingApplications : winnersApplications).map(
            (app: any) => (
              <ListItem
                disableGutters
                key={app.id}
